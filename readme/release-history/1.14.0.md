...
The `DataNavigator` fluent API gains full JSONPath-style expression support in `get()`, `has()`, and the new `query()` method for multiple-return scenarios.
...
// New query() method returns all matches as a BoxLang Array
results = nav.query( "store.products[?(@.price > 100)].name" )
```

The new `getOrDefault( key, defaultValue )` method provides safe navigation with fallbacks, complementing the existing `getOrThrow()` for strict access patterns.
