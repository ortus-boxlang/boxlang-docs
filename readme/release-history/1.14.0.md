...
BoxLang now treats **class references** — both Java and BoxLang — as first-class callable objects. This means you can call `.init()` directly on an imported class without using the `new` operator, and you can use a functional approach to object creation by invoking the class reference itself as a function.

**`.init()` on imported classes** — call `.init()` on any imported class to invoke its constructor. This works on both BoxLang and Java classes, and on any expression that evaluates to a class:

```js
// BoxLang class — no 'new' keyword needed
import models.User
...
// Functional object creation — all equivalent:
