...
Full reference: [Encrypted Configuration Secrets](../../getting-started/configuration/security.md#encrypted-configuration-secrets-bxsecret). For the deep dive, see the community post: [Encrypted Configuration Values with bxsecret: — BoxLang 1.17.0](https://community.ortussolutions.com/t/encrypted-configuration-values-with-bxsecret-boxlang-1-17-0/11161).
...
* [**BL-2629**](https://ortussolutions.atlassian.net/browse/BL-2629) — `DynamicClassLoader` no longer locks the original JAR files it loads (including JARs referenced by `javaSettings` in `Application.bx`) — each JAR is copied to a temp file before loading, preventing Windows file-lock issues on hot-reload. Controlled by the `jarTempFileCaching` setting (default `true`). Full details: [Java Integration → No More Locked JARs](../../boxlang-framework/java-integration.md#no-more-locked-jars-jartempfilecaching), and the community post [No More Locked JARs: jarTempFileCaching — BoxLang 1.17.0](https://community.ortussolutions.com/t/no-more-locked-jars-jartempfilecaching-boxlang-1-17-0/11162).
...
* [**BL-2646**](https://ortussolutions.atlassian.net/browse/BL-2646) — Added an automatic background watchdog that evicts the parser's ANTLR DFA cache using three independent triggers — heap pressure, idle time, and max age — preventing long-running server processes from accumulating the 400MB-1GB of unbounded parser memory this cache could reach. Controlled by the experimental flag `experimental.clearParserCache` (default `true`). Full details: [Experimental → Clear Parser Cache](../../getting-started/configuration/experimental.md#clear-parser-cache), and the community post [Automatic ANTLR DFA Cache Eviction — BoxLang 1.17.0](https://community.ortussolutions.com/t/automatic-antlr-dfa-cache-eviction-boxlang-1-17-0/11163).
...
The following patches were released for 1.17.x
...
* [BL-2667](https://ortussolutions.atlassian.net/browse/BL-2667https:/ortussolutions.atlassian.net/browse/BL-2666https:/ortussolutions.atlassian.net/browse/BL-2665) - file component action="write" is supposed to default relative paths to tmp dir
* [BL-2666](https://ortussolutions.atlassian.net/browse/BL-2667https:/ortussolutions.atlassian.net/browse/BL-2666https:/ortussolutions.atlassian.net/browse/BL-2665) - Improve listAppend() performance when includeEmptyFields=false
* [BL-2665](https://ortussolutions.atlassian.net/browse/BL-2667https:/ortussolutions.atlassian.net/browse/BL-2666https:/ortussolutions.atlassian.net/browse/BL-2665) - Adobe doesn't convert query nulls to empty string on JSON serialization
